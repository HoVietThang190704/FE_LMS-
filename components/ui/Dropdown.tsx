...
export type DropdownProps = {
  /**
   * trigger can be a node or a render function that receives the `open` state.
   * Using a render function enables UI changes based on whether the dropdown is open.
   */
...
export function DropdownItem({ children, onClick, href, className = '' }: {
  ...
  if (href) {
    // Use next/link lazily in caller to avoid coupling, but allow simple anchor
    return (
