...
export default function CourseCard({ course, enrolled = false, messages = {}, locale = 'vi' }: Props) {
  ...
  return (
    <>
      <Link href={ROUTES.COURSE_DETAIL(String(course.id))} className="block group">
        <article className="bg-white rounded-3xl gap-4 border border-gray-200 hover:shadow-md transition-shadow overflow-hidden cursor-pointer relative">
          ...
          <div className="p-5 flex flex-col justify-between min-h-[220px]">
            ...
            <div className="mt-4 flex flex-col gap-2">
              <div className="block text-center bg-black text-white rounded-md py-2 group-hover:bg-gray-900 transition">
                {t('courseCard.enter','Vào lớp học')}
              </div>
              {!enrolled && (
                <button
                  type="button"
                  className={`block w-full text-center rounded-md py-2 font-semibold ${isFull ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'} transition`}
